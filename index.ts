...
async function main() {
  ...
  const inPrevConsensus = existingCg.filter((v) => validatorNames.includes(v));
  console.log({ inPrevConsensus });

  // check if any of our validators are in current consensus group
  const inConsensus = cg.filter((v) => validatorNames.includes(v.name));
  console.log({ inConsensus });
